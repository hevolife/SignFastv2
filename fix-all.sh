...
#############################################
# 🔧 CORRECTION COMPLÈTE NGINX + BUILD
#############################################
...
echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}🔧 CORRECTION NGINX + BUILD SIGNFAST${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo ""
...
# 4. Vérifier si le code source existe
echo -e "${YELLOW}4️⃣ Vérification du code source...${NC}"
if [ ! -d "/var/www/signfast" ]; then
    echo -e "${RED}❌ /var/www/signfast n'existe pas${NC}"
    echo ""
    echo -e "${YELLOW}📋 OPTIONS :${NC}"
    echo "A) Cloner depuis GitHub (recommandé)"
...
cd /var/www/signfast
echo -e "${GREEN}✅ Code source présent${NC}"
echo ""
...
if sudo nginx -T 2>/dev/null | grep -q "root /var/www/signfast/dist"; then
    echo -e "${GREEN}✅ Nginx pointe vers /var/www/signfast/dist${NC}"
else
